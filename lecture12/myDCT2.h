...
void Quantization(double input[8][8], double output[8][8], double val){
	double q = val;

	for(int l = 0; l < 8; l++){
		for(int k = 0; k < 8; k++){

			output[k][l] = round(input[k][l]/(q*QTable[k][l]));

		}
	}
}

void Reconstruction(double input[8][8], double output[8][8], double val){
	double q = val;

	for(int l = 0; l < 8; l++){
		for(int k = 0; k < 8; k++){

			output[k][l] = input[k][l] * (q*QTable[k][l]);
